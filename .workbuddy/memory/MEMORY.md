...
## 国密算法真实现 (2026-07-14, 已完成)
crypto.rs 中 SM2/SM3/SM4 已从假实现替换为 libsm crate 真实现:
- **SM3**: 已使用 libsm (无需改), SM3("abc") 匹配 GM/T 0004-2012 标准测试向量
- **SM2 签名/验签**: 使用 `libsm::sm2::signature::SigCtx`
  - 首次调用时随机生成 SM2 密钥对 (sm2p256v1 曲线), 缓存在 `OnceLock<Sm2KeyPair>`
  - 签名输出 DER 编码 (ASN.1 SEQUENCE { r, s }), 70-72 字节, 符合 GM/T 0003-2012
  - 旧假实现返回 64 字节裸 r||s, 新实现格式不同但所有调用方仅做透传 (DER BIT STRING / base64), 无需适配
- **SM4 加密/解密**: 使用 `libsm::sm4::{Cipher, Mode::Cbc}` 真实 CBC 分组密码, PKCS#7 自动填充
- **RSA 签名**: 保持假实现 (非国密, 仅兼容用)
- lib.rs 新增 WASM 导出: `sm3_hash`, `sm2_sign`, `sm2_verify`, `get_sm2_pubkey`, `sm4_encrypt`, `sm4_decrypt`
- 测试脚本: `tools/test_crypto_real.cjs` (Node.js, 18/18 通过)
- 无需新增 Cargo.toml 依赖, libsm 0.6.1 已在依赖中
