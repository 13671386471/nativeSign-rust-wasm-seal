...
| 算法名称 | 算法类型 | 核心作用 (做什么) | 通俗理解 (像什么) | 主要特点 |
|----------|----------|-------------------|-------------------|----------|
| SM2 | 非对称加密算法 | 数字签名、身份认证、密钥交换 | 数字世界的"身份证"和"印章" | 基于椭圆曲线(ECC)，256位密钥，安全强度高、运算快 |
| SM3 | 密码杂凑(哈希)算法 | 为任意数据生成独一无二的"数字指纹"，用于完整性验证 | 数据的"防伪指纹"或"校验章" | 生成256位哈希值，安全强度高于SHA-256 |
| SM4 | 对称加密算法 | 对大量数据进行快速的加密和解密，保证数据机密性 | 数据的"保险柜" | 分组密码，密钥和分组长度均为128位，与AES相当 |
